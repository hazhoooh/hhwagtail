...
class ImagesPermissionPolicyGetter:
    ...
    """
    def __get__(self, obj, objtype=None): return permission_policy

def set_permission_policy():
    global permission_policy
    permission_policy = CollectionOwnershipPermissionPolicy(get_image_model(), auth_model=Image, owner_field_name="uploaded_by_user")

# @receiver(setting_changed)
# def update_permission_policy(signal, sender, setting, **kwargs):
#     """ Updates the permission policy when the `CONSOLE_IMAGES_IMAGE_MODEL` setting changes.
#         This is useful in tests where we override the base image model and expect the permission policy to have changed accordingly.
#     """
#     if setting == "CONSOLE_IMAGES_IMAGE_MODEL": set_permission_policy()
