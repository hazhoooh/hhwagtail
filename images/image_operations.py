...
class Operation:
    def __init__(self, method, *args):
        ...
        self.args = args
        # Check arguments
        try: inspect.getcallargs(self.construct, *args)
        except TypeError as e: raise InvalidFilterSpecError(e)
        # Call construct
        try: self.construct(*args)
        except ValueError as e: raise InvalidFilterSpecError(e)
...
class FillOperation(TransformOperation):
    ...
    def construct(self, size, *extra):
        ...
        for extra_part in extra:
            if extra_part.startswith('c'): self.crop_closeness = int(extra_part[1:])
            else: raise ValueError("Unrecognised filter spec part: %s" % extra_part)

        # Divide it by 100 (as it's a percentage)
        self.crop_closeness /= 100

        # Clamp it
        if self.crop_closeness > 1: self.crop_closeness = 1
    ...
    def run(self, transform, image):
        ...
        if focal_point is not None: rect = rect.move_to_cover(focal_point)
