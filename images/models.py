    ...
    def get_file_size(self):
        if self.file_size is None:
            try: self.file_size = self.file.size
                ...
                # File not found Have to catch everything, because the exception depends on the file subclass, and therefore the storage being used.
...
class AbstractImage(ImageFileMixin, CollectionMember, index.Indexed, models.Model):
    ...
    file = models.ImageField(verbose_name="file", upload_to=get_upload_to, width_field='width', height_field='height')
    ...
    uploaded_by_user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name="uploaded by user",null=True, blank=True, editable=False, on_delete=models.SET_NULL)
    tags = TaggableManager(help_text=None, blank=True, verbose_name="tags")
    ...
    def get_file_hash(self):
        if self.file_hash == '':
            with self.open_file() as f: self._set_file_hash(f.read())
    ...
    def get_upload_to(self, filename):
        ...
        if len(full_path) >= 95:
            ...
            full_path = os.path.join(folder_name, filename)
        return full_path
    def get_usage(self): return get_object_usage(self)
    @property
    def usage_url(self): return reverse('wagtailimages:image_usage',args=(self.id,))
    search_fields = CollectionMember.search_fields + [
        index.SearchField('title', partial_match=True, boost=10),
        index.AutocompleteField('title'),
        index.FilterField('title'),
        index.RelatedFields('tags', [index.SearchField('name', partial_match=True, boost=10),index.AutocompleteField('name'),]),
        index.FilterField('uploaded_by_user'),
    ]
    def __str__(self):return self.title
    def get_rect(self):return Rect(0, 0, self.width, self.height)
    def get_focal_point(self):
        if self.focal_point_x is not None and self.focal_point_y is not None and self.focal_point_width is not None and self.focal_point_height is not None:
            return Rect.from_point(self.focal_point_x,self.focal_point_y,self.focal_point_width,self.focal_point_height,)
    def has_focal_point(self): return self.get_focal_point() is not None
    ...
    @classmethod
    def get_rendition_model(cls): return cls.renditions.rel.related_model
    def get_rendition(self, filter):
        if isinstance(filter, str): filter = Filter(spec=filter)
        cache_key = filter.get_cache_key(self)
        Rendition = self.get_rendition_model()
        try:
            rendition_caching = True
            cache = caches['renditions']
            rendition_cache_key = Rendition.construct_cache_key(self.id,cache_key,filter.spec)
            cached_rendition = cache.get(rendition_cache_key)
            if cached_rendition: return cached_rendition
        except InvalidCacheBackendError: rendition_caching = False

        try: rendition = self.renditions.get(filter_spec=filter.spec,focal_point_key=cache_key,)
        except Rendition.DoesNotExist:
            try:
                logger.debug("Generating '%s' rendition for image %d",filter.spec,self.pk,)
                start_time = time.time()
                generated_image = filter.run(self, BytesIO())
                logger.debug("Generated '%s' rendition for image %d in %.1fms",filter.spec,self.pk,(time.time() - start_time) * 1000)
                print(f"Generated '{filter.spec}' rendition for image {self.pk} in {(time.time() - start_time) * 1000}ms")
            except:
                logger.debug("Failed to generate '%s' rendition for image %d", filter.spec, self.pk)
                print(f"Failed to generate '{filter.spec}' rendition for image {self.pk}")
                raise
            input_filename = os.path.basename(self.file.name)
            input_filename_without_extension, input_extension = os.path.splitext(input_filename)
            FORMAT_EXTENSIONS = {'jpeg': '.jpg','png': '.png','gif': '.gif','webp': '.webp',}
            output_extension = filter.spec.replace('|', '.') + FORMAT_EXTENSIONS[generated_image.format_name]
            if cache_key: output_extension = cache_key + '.' + output_extension
            output_filename_without_extension = input_filename_without_extension[:(59 - len(output_extension))]
            output_filename = output_filename_without_extension + '.' + output_extension
            rendition, created = self.renditions.get_or_create(filter_spec=filter.spec,focal_point_key=cache_key,defaults={'file': File(generated_image.f, name=output_filename)})
        if rendition_caching: cache.set(rendition_cache_key, rendition)
        return rendition
    def is_portrait(self): return (self.width < self.height)
    def is_landscape(self): return (self.height < self.width)

    @property
    def filename(self): return os.path.basename(self.file.name)

    @property
    def default_alt_text(self): return self.title

    def is_editable_by_user(self, user):
        from wagtail.images.permissions import permission_policy
        return permission_policy.user_has_permission_for_instance(user, 'change', self)
    def to_json(self): return {'id': self.id,'title': self.title,'file': self.file.url,'width': self.width,'height': self.height,'created_at': self.created_at,'uploaded_by_user': self.uploaded_by_user,'tags': self.tags,'file_hash': self.file_hash}
    class Meta: abstract = True

class Image(AbstractImage):
    admin_form_fields = ('title','file','collection','tags','focal_point_x','focal_point_y','focal_point_width','focal_point_height',)
...
class Filter:
    ...
    def transform_operations(self):
        return [operation for operation in self.operations if isinstance(operation, TransformOperation)]

    @property
    def filter_operations(self):
        return [operation for operation in self.operations if isinstance(operation, FilterOperation)]
    ...
    def run(self, image, output):
        with image.get_willow_image() as willow:
            ...
            if 'output-format' in env:
                ...
                conversion = getattr(settings, 'CONSOLE_IMAGES_FORMAT_CONVERSIONS', {})
            ...
            if output_format == 'jpeg':
                ...
                if 'jpeg-quality' in env:quality = env['jpeg-quality']
                else:quality = getattr(settings, 'CONSOLE_IMAGES_JPEG_QUALITY', 85)
                ...
                if ('output-format-options' in env and 'lossless' in env['output-format-options']):
                    return willow.save_as_webp(output, lossless=True)
                elif 'webp-quality' in env:quality = env['webp-quality']
                else:quality = getattr(settings, 'CONSOLE_IMAGES_WEBP_QUALITY', 85)
