...
def get_image_model_string():
    ...
    """
    return getattr(settings, 'CONSOLE_IMAGES_IMAGE_MODEL', 'wagtailimages.Image')


def get_image_model():
    """
    Get the image model from the ``CONSOLE_IMAGES_IMAGE_MODEL`` setting.
    ...
    try:
        ...
        raise ImproperlyConfigured("CONSOLE_IMAGES_IMAGE_MODEL must be of the form 'app_label.model_name'")
    except LookupError:
        raise ImproperlyConfigured(
            "CONSOLE_IMAGES_IMAGE_MODEL refers to model '%s' that has not been installed" % model_string
        )
