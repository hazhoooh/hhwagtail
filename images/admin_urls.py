...
from wagtail.images.views import api_by_hh, chooser, images, multiple


app_name = 'wagtailimages'
urlpatterns = [
    path('', images.IndexView.as_view(), name='index'),
    path('list/', api_by_hh.ImageListViewSet.as_view({"get": "list", "post": "create"}), name='index_list'),
    # path('json_results/', images.ListingResultsView.as_view(), name='json_listing_results'),
    ...
    # path('multiple/create_from_uploaded_image/<int:uploaded_image_id>/', multiple.CreateFromUploadedImageView.as_view(), name='create_multiple_from_uploaded_image'),
    # path('multiple/<int:image_id>/delete/', multiple.DeleteView.as_view(), name='delete_multiple'),
