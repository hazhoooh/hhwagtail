...
CHOOSER_PAGE_SIZE = getattr(settings, 'CONSOLE_IMAGES_CHOOSER_PAGE_SIZE', 12)
...
def get_image_result_data(image):
    ...
    preview_image = image.get_rendition('max-50x50')
    return {'id': image.id,'edit_link': reverse('wagtailimages:edit', args=(image.id,)),'title': image.title,'preview': {'url': preview_image.url,'width': preview_image.width,'height': preview_image.height,}}

class BaseChooseView(View):
    def get(self, request):
        self.image_model = get_image_model()
        images = permission_policy.instances_user_has_any_permission_for(request.user, ['choose']).order_by('-created_at')
        for hook in hooks.get_hooks('construct_image_chooser_queryset'): images = hook(images, request)
        collection_id = request.GET.get('collection_id')
        if collection_id: images = images.filter(collection=collection_id)
        ...
        else: self.search_form = SearchForm()
        if not self.is_searching:
            tag_name = request.GET.get('tag')
            if tag_name: images = images.filter(tags__name=tag_name)
        paginator = Paginator(images, per_page=CHOOSER_PAGE_SIZE)
        self.images = paginator.get_page(request.GET.get('p'))
        return self.render_to_response()
    def get_context_data(self): return {'images': self.images,'is_searching': self.is_searching,'query_string': self.q,'will_select_format': self.request.GET.get('select_format')}
    def render_to_response(self): raise NotImplementedError()
...
class ChooseView(BaseChooseView):
    def get_context_data(self):
        ...
        else: uploadform = None
        collections = permission_policy.collections_user_has_permission_for(self.request.user, 'choose')
        if len(collections) < 2: collections = None
        context.update({'searchform': self.search_form,'popular_tags': popular_tags_for_model(self.image_model),'collections': collections,'uploadform': uploadform,})
        return context
    ...
    def render_to_response(self):
        return render_modal_workflow(
            ...
            json_data={
                ...
                'error_message': "Report this error to your website administrator with the following information:",
                'tag_autocomplete_url': reverse('tags_autocomplete_url'),
            }
        )

class ChooseResultsView(BaseChooseView):
    def render_to_response(self): return TemplateResponse(self.request, "wagtailimages/chooser/results.html", self.get_context_data())

def image_chosen(request, image_id):
    image = get_object_or_404(get_image_model(), id=image_id)
    return render_modal_workflow(request, None, None,None, json_data={'step': 'image_chosen', 'result': get_image_result_data(image)})
