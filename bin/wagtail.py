#!/usr/bin/env python
import fileinput, fnmatch, os, re, sys
...
def pluralize(value, arg='s'): return '' if value == 1 else arg
...
class CreateProject(Command):
    ...
    def run(self, project_name=None, dest_dir=None):
        ...
        try: __import__(project_name)
        except ImportError: pass
        else: sys.exit("'%s' conflicts with the name of an existing Python module and cannot be used as a project name. Please try another name." % project_name)
        ...
        utility_args = ['django-admin','startproject','--template=' + template_path,'--ext=html,rst','--name=Dockerfile',project_name]
        if dest_dir: utility_args.append(dest_dir)
...
class UpdateModulePaths(Command):
    ...
    def run(self, root_path=None, list_files=False, diff=False, ignored_dirs=None, ignored_patterns=None):
        ...
        if ignored_patterns is None: ignored_patterns = []
        checked_file_count = 0
        changed_file_count = 0

        for (dirpath, dirnames, filenames) in os.walk(root_path):
            dirpath_with_slash = os.path.abspath(dirpath) + os.sep
            if any(dirpath_with_slash.startswith(ignored_dir) for ignored_dir in absolute_ignored_dirs): continue
            for filename in filenames:
                if not filename.lower().endswith('.py'): continue
                if any(fnmatch.fnmatch(filename, pattern) for pattern in ignored_patterns): continue
                path = os.path.join(dirpath, filename)
                relative_path = os.path.relpath(path, start=root_path)
                checked_file_count += 1

                if diff: change_count = self._show_diff(path, relative_path=relative_path)
                else:
                    if list_files: change_count = self._count_changes(path)
                    else: change_count = self._rewrite_file(path)
                    if change_count: print("%s - %d change%s" % (relative_path, change_count, pluralize(change_count)))
                if change_count: changed_file_count += 1
        if diff or list_files: print("\nChecked %d .py file%s, %d file%s to update." % (checked_file_count, pluralize(checked_file_count),changed_file_count, pluralize(changed_file_count)))
    ...
    def _rewrite_line(self, line):
        for pattern, repl in self.REPLACEMENTS: line = re.sub(pattern, repl, line)
        return line
...
def prog_name(): return os.path.basename(sys.argv[0])
def help_index():
    print("Type '%s help <subcommand>' for help on a specific subcommand.\n" % prog_name())
    print("Available subcommands:\n")
    for name, cmd in sorted(COMMANDS.items()): print("    %s%s" % (name.ljust(20), cmd.description))
    return
def unknown_command(command):
    ...
    return
def main():
    try: command_name = sys.argv[1]
    except IndexError: help_index()
    if command_name == 'help':
        try: help_command_name = sys.argv[2]
        except IndexError: help_index()
        try: command = COMMANDS[help_command_name]
        except KeyError: unknown_command(help_command_name)
        command.print_help(help_command_name)
        return
    try: command = COMMANDS[command_name]
    except KeyError: unknown_command(command_name)
