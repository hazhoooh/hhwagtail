...
class SafePaginateListView(ListView):
    ...
    def paginate_queryset(self, queryset, page_size):
        ...
        try: page_number = int(page_request)
        except ValueError:
            if page_request == 'last': page_number = paginator.num_pages
            else: page_number = 0
        try:
            if page_number > paginator.num_pages: page_number = paginator.num_pages  # page out of range, show last page
...
class FormPagesListView(SafePaginateListView):
    """ Lists the available form pages for the current user (model property have not been set because we want to rely on the page permissions policies, this way we don't need to create special permissions policies for FormPages separately) """
...
class DeleteSubmissionsView(TemplateView):
    ...
    def get_queryset(self):
        ...
        submission_ids = self.request.GET.getlist('ss')
...
class SubmissionsListView(SpreadsheetExportMixin, SafePaginateListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not get_forms_for_user(request.user).filter(pk=self.form_page.id).exists(): raise PermissionDenied
    ...
    def get_queryset(self):
        ...
        if filtering and isinstance(filtering, dict): queryset = queryset.filter(**filtering)
        return queryset
