...
def purge_urls_from_cache(urls, backend_settings=None, backends=None):
    # Convert each url to urls one for each managed language (VISITORS_END_CACHE_LANGUAGES setting).
    # The managed languages are common to all the defined backends.
    # This depends on settings.USE_I18N
    # If CONSOLE_I18N_ENABLED is True, this defaults to CONSOLE_CONTENT_LANGUAGES
    wagtail_i18n_enabled = getattr(settings, 'CONSOLE_I18N_ENABLED', False)
    content_languages = get_content_languages() if wagtail_i18n_enabled else {}
    languages = getattr(settings, 'VISITORS_END_CACHE_LANGUAGES', list(content_languages.keys()))
...
class PurgeBatch:
    ...
    def purge(self, backend_settings=None, backends=None):
        ...
        - backend_settings can be used to override the FRONTENDCACHE setting for
          just this call
