...
class Redirect(models.Model):
    old_path = models.CharField(verbose_name="redirect from", max_length=255, db_index=True)
    site = models.ForeignKey('wagtailcore.Site',verbose_name="site",null=True, blank=True,related_name='redirects',db_index=True,on_delete=models.CASCADE)
    ...
    redirect_page = models.ForeignKey('wagtailcore.Page',verbose_name="redirect to a page",null=True, blank=True,on_delete=models.CASCADE)
    redirect_page_route_path = models.CharField(verbose_name="target page route",help_text="Optionally specify a route on the target page to redirect to. Leave blank to redirect to the default page route.",blank=True,max_length=255)
    redirect_link = models.CharField(verbose_name="redirect to any URL", blank=True, max_length=255)
    automatically_created = models.BooleanField(verbose_name="automatically created",default=False,editable=False,)
    created_at = models.DateTimeField(verbose_name="created at", auto_now_add=True, null=True)

    @property
    def title(self): return self.old_path
    ...
    def link(self):
        if self.redirect_page:
            ...
            base_url = page.url
            if not self.redirect_page_route_path: return base_url
            try: page.resolve_subpage(self.redirect_page_route_path)
            except (AttributeError, Resolver404): return base_url
            return base_url.rstrip("/") + self.redirect_page_route_path
        elif self.redirect_link: return self.redirect_link
        return None
    ...
    def get_for_site(cls, site=None):
        if site: return cls.objects.filter(models.Q(site=site) | models.Q(site=None))
        else: return cls.objects.all()
    ...
    def add_redirect(old_path, redirect_to=None, is_permanent=True, page_route_path=None, site=None, automatically_created=False):
        ...
        if isinstance(redirect_to, Page):
            ...
            if isinstance(page_route_path, str): redirect.redirect_page_route_path = Redirect.normalise_page_route_path(page_route_path)
        elif isinstance(redirect_to, str): redirect.redirect_link = redirect_to # Set redirect link string
    ...
    def clean(self):
        ...
        if self.redirect_page: self.redirect_page_route_path = Redirect.normalise_page_route_path(self.redirect_page_route_path)
        else: self.redirect_page_route_path = ""
