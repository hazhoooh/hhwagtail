...
def index(request):
    ...
    # Pagination
    paginator = Paginator(redirects, per_page=150)
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        return TemplateResponse(request, "wagtailredirects/index.html", {
            ...
            'search_form': SearchForm(
                data=dict(q=query_string) if query_string else None, placeholder="Search redirects"
            ),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with transaction.atomic():
                form.save()
                log(instance=theredirect, action='edited')
            messages.success(request, "Redirect '{0}' updated.".format(theredirect.title), buttons=[
                messages.button(reverse('wagtailredirects:edit', args=(theredirect.id,)), "Edit")
            ])
...
def delete(request, redirect_id):
    ...
    if request.method == 'POST':
        with transaction.atomic():
            log(instance=theredirect, action='deleted')
            theredirect.delete()
        messages.success(request, "Redirect '{0}' deleted.".format(theredirect.title))
        return redirect('wagtailredirects:index')
...
def add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Redirect '{0}' added.".format(theredirect.title), buttons=[
                messages.button(reverse('wagtailredirects:edit', args=(theredirect.id,)), "Edit")
            ])
            return redirect('wagtailredirects:index')
        else:
...
def start_import(request):
    ...
    if extension not in supported_extensions:
        messages.error(
            request,
            "File format of type '{}' is not supported".format(extension)
        )
    ...
    try:
        ...
        messages.error(
            request,
            "Imported file has a wrong encoding: %s" % e
        )
        return redirect('wagtailredirects:start_import')
    except Exception as e:  # pragma: no cover
        messages.error(
            request,
            "%(error)s encountered while trying to read file: %(filename)s" % {
...
def create_redirects_from_dataset(dataset, config):
    ...
    for row in dataset:
        ...
        with transaction.atomic():
            redirect = form.save()
            log(instance=redirect, action='created')
        successes += 1
