...
DEFAULT_TABLE_OPTIONS = {
    'minSpareRows': 0,'startRows': 3,'startCols': 3,'colHeaders': False,'rowHeaders': False,
    'contextMenu': ['row_above','row_below','---------','col_left','col_right','---------','remove_row','remove_col','---------','undo','redo'],
    'editor': 'text','stretchH': 'all','height': 108,'renderer': 'text','autoColumnSize': False,
}
...
class TableInput(forms.HiddenInput):
    ...
    def media(self):
        return forms.Media(
            css={'all': [static('table_block/6.2.2.min.css') + '?v1',]},
            js=[static('table_block/6.2.2.min.js') + '?v1',static('table_block/table.js') + '?v2',]
        )

class TableInputAdapter(WidgetAdapter):
    js_constructor = 'wagtail.widgets.TableInput'
    def js_args(self, widget):
        strings = {'r_h': "Row header",'f_r_h': "Display the first row as column header.",'c_h': "Column header",'f_c_h': "Display the first column as the row header.",'t_c': "Table caption",'t_t': "Table Topic Heading",'Table': "Table",}
        return [widget.table_options,strings,]
...
class TableBlock(FieldBlock):
    ...
    @cached_property
    def field(self): return forms.CharField(widget=TableInput(table_options=self.table_options), **self.field_options)
    def value_from_form(self, value): return json.loads(value)
    def value_for_form(self, value): return json.dumps(value)
    def get_form_state(self, value): return value # pass state to frontend as a JSON-ish dict - do not serialise to a JSON string
    def is_html_renderer(self): return self.table_options['renderer'] == 'html'
    def get_searchable_content(self, value):
        content = []
        if value:
            for row in value.get('data', []): content.extend([v for v in row if v])
        return content
    def render(self, value, context=None):
        ...
        if template and value:
            ...
            if context is None: new_context = {}
            else: new_context = dict(context)
            new_context.update({'self': value,self.TEMPLATE_VAR: value,'table_header': table_header,'first_col_is_header': first_col_is_header,'html_renderer': self.is_html_renderer(),'table_caption': value.get('table_caption'),'data': value['data'][1:] if table_header else value.get('data', [])})
            if value.get('cell'):
                new_context['classnames'] = {}
                for meta in value['cell']:
                    if 'className' in meta: new_context['classnames'][(meta['row'], meta['col'])] = meta['className']
            return render_to_string(template, new_context)
        else: return self.render_basic(value or "", context=context)
