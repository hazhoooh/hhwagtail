...
from .registry import register_menu

__all__ = ['BaseSetting', 'register_menu']

class BaseSetting(models.Model):
    """ The abstract base model for settings. Subclasses must be registered using :func:`~wagtail.contrib.settings.registry.register_setting` """
    # Override to fetch ForeignKey values in the same query when retrieving settings via for_site()
    select_related = None
    ...
    def base_queryset(cls):
        """ Returns a queryset of objects of this type to use as a base for calling get_or_create() on.
            You can use the `select_related` attribute on your class to specify a list of foreign key field names, which the method will attempt to select additional related-object data for when the query is executed.
            If your needs are more complex than this, you can override this method on your custom class.
        """
        queryset = cls.objects.all()
        if cls.select_related is not None: queryset = queryset.select_related(*cls.select_related)
        return queryset
    @classmethod
    ...
    def for_request(cls, request):
        ...
        if hasattr(request, attr_name): return getattr(request, attr_name)
    ...
    def get_page_url(self, attribute_name, request=None):
        ...
        field or attribute.
        """
        if attribute_name in self._page_url_cache: return self._page_url_cache[attribute_name]
        if not hasattr(self, attribute_name): raise AttributeError("'{}' object has no attribute '{}'".format(self.__class__.__name__, attribute_name))
        page = getattr(self, attribute_name)
        if hasattr(page, 'specific'): url = page.specific.get_url(getattr(self, '_request', None))
        else: url = ""
        self._page_url_cache[attribute_name] = url
        return url
    def __str__(self): return "%s for %s" % (self._meta.verbose_name.capitalize(), self.site)
