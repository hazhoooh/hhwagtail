...
class StreamField(Field):
    ...
    """
    def to_representation(self, value): return value.stream_block.get_api_representation(value, self.context)
...
class TagsField(Field):
    ...
    "tags": ["bird", "wagtail"]
    """
    def to_representation(self, value): return list(value.all().order_by('name').values_list('name', flat=True))
...
class BaseSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if 'id' in [field.field_name for field in fields]: data['id'] = None
        # Serialise meta fields
        meta = OrderedDict()
        for field in meta_fields:
            try: attribute = field.get_attribute(instance)
            except SkipField: continue
            if attribute is None:
                # We skip `to_representation` for `None` values so that fields do not have to explicitly deal with that case.
                meta[field.field_name] = None
            else: meta[field.field_name] = field.to_representation(attribute) 
        if meta: data['meta'] = meta 
        # Serialise core fields
        for field in fields:
            try:
                if field.field_name == 'admin_display_title': instance = instance.specific_deferred
            ...
            else: data[field.field_name] = field.to_representation(attribute) 
        return data

    def build_property_field(self, field_name, model_class):
        # TaggableManager is not a Django field so it gets treated as a property
        field = getattr(model_class, field_name)
        if isinstance(field, _TaggableManager): return TagsField, {}
...
class PageSerializer(BaseSerializer):
    ...
    def build_relational_field(self, field_name, relation_info):
        ...
        if relation_info.to_many:
            ...
            child_relations = {child_relation.field.remote_field.related_name: child_relation.related_model for child_relation in get_all_child_relations(model)}
...
def get_serializer_class(model, field_names, meta_fields, field_serializer_overrides=None, child_serializer_classes=None, base=BaseSerializer):
    ...
    attrs = {'Meta': Meta,'meta_fields': list(meta_fields),'child_serializer_classes': child_serializer_classes or {},}
    if field_serializer_overrides: attrs.update(field_serializer_overrides)
