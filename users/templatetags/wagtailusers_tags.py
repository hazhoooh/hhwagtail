...
def format_permissions(permission_bound_field):
    ...
    # checkbox objects
    checkboxes_by_id = {int(checkbox.data['value'].value): checkbox for checkbox in permission_bound_field} # checkbox.data['value'] gives a ModelChoiceIteratorValue
...
def group_listing_buttons(context, group):
    button_hooks = hooks.get_hooks('register_group_listing_buttons')
    buttons = sorted(itertools.chain.from_iterable(
        hook(context, group)
        for hook in button_hooks))
