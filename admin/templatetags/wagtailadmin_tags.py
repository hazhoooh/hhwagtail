...
from wagtail.core.models import Collection, CollectionViewRestriction, Page, PageViewRestriction, UserPagePermissionsProxy
...
def menu_search(context):
    ...
    if not search_areas: return ''
    search_area = search_areas[0]
    return render_to_string('cms/shared/menu_search.html', {'search_url': search_area.url,})

@register.inclusion_tag('cms/shared/main_nav.html', takes_context=True)
def main_nav(context):
    request = context['request']
    return {'menu_html': admin_menu.render_html(request),'request': request,}

@register.inclusion_tag('cms/shared/breadcrumb.html', takes_context=True)
def explorer_breadcrumb(context, page=None, include_self=True):
    user = context['request'].user
    # find the closest common ancestor of the pages that this user has direct explore permission (i.e. add/edit/publish/lock) over; this will be the root of the breadcrumb
    explorable_root_page = get_explorable_root_page(user)
    if not explorable_root_page or not page: return {'pages': Page.objects.none()}
    return {'pages': page.get_ancestors(inclusive=include_self).descendant_of(explorable_root_page, inclusive=True).specific()}

@register.inclusion_tag('cms/shared/search_other.html', takes_context=True)
...
def fieldtype(bound_field):
    try: return camelcase_to_underscore(bound_field.field.__class__.__name__)
    except AttributeError:
        try: return camelcase_to_underscore(bound_field.__class__.__name__)
        except AttributeError: return ""

@register.filter
def widgettype(bound_field):
    try: return camelcase_to_underscore(bound_field.field.widget.__class__.__name__)
    except AttributeError:
        try: return camelcase_to_underscore(bound_field.widget.__class__.__name__)
        except AttributeError: return ""

def _get_user_page_permissions(context):
    # Create a UserPagePermissionsProxy object to represent the user's global permissions, and cache it in the context for the duration of the page request, if one does not exist already
    if 'user_page_permissions' not in context: context['user_page_permissions'] = UserPagePermissionsProxy(context['request'].user)
...
def test_collection_is_public(context, collection):
    ...
    """
    if 'all_collection_view_restrictions' not in context: context['all_collection_view_restrictions'] = CollectionViewRestriction.objects.select_related('collection').values_list('collection__name', flat=True)
...
def test_page_is_public(context, page):
    """
        ...
        DB queries on repeated calls.
    """
    if not hasattr(context["request"], "all_page_view_restriction_paths"): context['request'].all_page_view_restriction_paths = PageViewRestriction.objects.select_related('page').values_list('page__path', flat=True)
...
def hook_output(hook_name):
    ...
    """
    return mark_safe(''.join([fn() for fn in hooks.get_hooks(hook_name)]))

@register.simple_tag
def base_url_setting(): return getattr(settings, 'BASE_URL', None)

@register.simple_tag
def allow_unicode_slugs(): return getattr(settings, 'CONSOLE_ALLOW_UNICODE_SLUGS', True)

@register.simple_tag
def auto_update_preview(): return getattr(settings, 'CONSOLE_AUTO_UPDATE_PREVIEW', False)
...
def render_with_errors(bound_field):
    ...
    if bound_field.errors and hasattr(widget, 'render_with_errors'): return widget.render_with_errors(bound_field.html_name,bound_field.value(),attrs={'id': bound_field.auto_id},errors=bound_field.errors)
    else: return bound_field.as_widget()
...
def querystring(context, **kwargs):
    """ Print out the current querystring. Any keyword arguments to this template tag will be added to the querystring before it is printed out.
        <a href="/page/{% querystring key='value' %}"> Will result in something like: <a href="/page/?foo=bar&key=value"> """
    request = context['request']
    querydict = request.GET.copy()
    # Can't do querydict.update(kwargs), because QueryDict.update() appends to the list of values, instead of replacing the values.
    for key, value in kwargs.items():
        if value is None: querydict.pop(key, None)
        else: querydict[key] = str(value)
    return '?' + querydict.urlencode()

@register.simple_tag(takes_context=True)
def table_header_label(context, label=None, sortable=True, ordering=None,sort_context_var='ordering', sort_param='ordering', sort_field=None,ascending_title_text=None, descending_title_text=None):
    if not sortable or not sort_field: return label
    if ordering is None: ordering = context.get(sort_context_var)
    reverse_sort_field = "-%s" % sort_field
    if ordering == sort_field:
        attrs = {'href': querystring(context, **{sort_param: reverse_sort_field}),'class': "icon icon-▼-after teal",}
        if descending_title_text is not None: attrs['title'] = descending_title_text
    elif ordering == reverse_sort_field:
        attrs = {'href': querystring(context, **{sort_param: sort_field}),'class': "icon icon-arrow-up-after teal",}
        if ascending_title_text is not None: attrs['title'] = ascending_title_text
    else:
        attrs = {'href': querystring(context, **{sort_param: sort_field}),'class': "icon icon-▼-after",}
        if ascending_title_text is not None: attrs['title'] = ascending_title_text
    attrs_string = format_html_join(' ', '{}="{}"', attrs.items())
    return format_html('<a {attrs}> {label} </a>',attrs=attrs_string, label=label)
...
@register.inclusion_tag("cms/shared/pagination_nav.html",takes_context=True)
def paginate(context, page, base_url='', page_key='p',classnames=''):
    """
    Print pagination previous/next links, and the page count. Take the
    following arguments:

    page: The current page of results. This should be a Django pagination `Page`
        instance

    base_url: The base URL of the next/previous page, with no querystring.
        This is optional, and defaults to the current page by just printing the
        querystring for the next/previous page.

    page_key: The name of the page variable in the query string. Defaults to 'p'.

    classnames: Extra classes to add to the next/previous links.
    """
    request = context['request']
    return {'base_url': base_url,'classnames': classnames,'request': request,'items': page,'page_key': page_key,'paginator': page.paginator,}

@register.inclusion_tag("cms/pages/listing/_buttons.html",takes_context=True)
def page_listing_buttons(context, page, page_perms, is_parent=False):
    ...
    buttons = []
    for hook in button_hooks: buttons.extend(hook(page, page_perms, is_parent, next_url))
    buttons.sort()
    for hook in hooks.get_hooks('construct_page_listing_buttons'): hook(buttons, page, page_perms, is_parent, context)
...
def message_tags(message):
    ...
    if message.extra_tags and level_tag: return message.extra_tags + ' ' + level_tag
    elif message.extra_tags: return message.extra_tags
    elif level_tag: return level_tag
    else: return ''

@register.filter('abs')
def _abs(val): return abs(val)

@register.filter
def admin_urlquote(value): return quote(value)

@register.simple_tag
def avatar_url(user_or_uid, size=50, gravatar_only=False):
    from django.contrib.auth import get_user_model
    if isinstance(user_or_uid, int):
        try: user = get_user_model().objects.get(id=user_or_uid)
        except get_user_model().DoesNotExist: return static('svg/def_avatar.svg') + "?v2"
    else: user = user_or_uid
    if hasattr(user, 'wagtail_userprofile') and user.wagtail_userprofile.avatar: return user.wagtail_userprofile.avatar.url
    return static('svg/def_avatar.svg') + "?v2"

@register.simple_tag
def js_translation_strings(): return {} # mark_safe(json.dumps(get_js_translation_strings()))

@register.simple_tag
def notification_static(path):
    """ Variant of the {% static %}` tag for use in notification emails - tries to form a full URL using BASE_URL if the static URL isn't already a full URL. """
...
@register.inclusion_tag("cms/shared/icon.html", takes_context=False)
...
@register.simple_tag
def resolve_url(url):
    ''' Used by cms/shared/pagination_nav.html'''
    if not url: return ''
    try: return resolve_url_func(url)
    except NoReverseMatch: return ''

@register.simple_tag(takes_context=True)
def component(context, obj, fallback_render_method=False):
    has_render_html_method = hasattr(obj, 'render_html')
    if not has_render_html_method and hasattr(obj, 'render'): return obj.render()
    elif not has_render_html_method: raise ValueError("Cannot render %r as a component" % (obj,))
