...
def send_mail(subject, message, recipient_list, from_email=None, **kwargs):
    ...
    """
    if not from_email:
        if hasattr(settings, 'CONSOLE_NOTIFICATION_FROM_EMAIL'):
            from_email = settings.CONSOLE_NOTIFICATION_FROM_EMAIL
...
def send_moderation_notification(revision, notification, excluded_user=None):
    ...
    if notification == 'submitted':
        ...
        include_superusers = getattr(settings, 'CONSOLE_NOTIFICATION_INCLUDE_SUPERUSERS', True)
...
def send_notification(recipient_users, notification, extra_context):
    ...
    # Get template
    template_subject = 'cms/notifications/' + notification + '_subject.txt'
    template_text = 'cms/notifications/' + notification + '.txt'
    template_html = 'cms/notifications/' + notification + '.html'
    ...
    with OpenedConnection(connection) as open_connection:
        ...
        for recipient in email_recipients:
            try:
                ...
                kwargs = {}
                if getattr(settings, 'CONSOLE_NOTIFICATION_USE_HTML', False):
...
class Notifier:
    ...
    notification = ''
    template_directory = 'cms/notifications/'
