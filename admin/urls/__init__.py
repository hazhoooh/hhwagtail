...
urlpatterns = [
    ...
    path('test404/', TemplateView.as_view(template_name='cms/404.html')),
    ...
    path('tag-autocomplete/', tags.autocomplete, name='tags_autocomplete_url'),

    path('collections/', include(wagtailadmin_collections_urls, namespace='wagtailadmin_collections')),
    path('workflows/', include(wagtailadmin_workflows_urls, namespace='wagtailadmin_workflows')),
    path('reports/', include(wagtailadmin_reports_urls, namespace='wagtailadmin_reports')),

    path('account/<int:uid>/', account.account, name='wagtailadmin_account'),
...
def get_sprite_hash():
    ...
    if not sprite_hash:
        ...
        sprite_hash = hashlib.sha1((content + settings.SECRET_KEY).encode("utf-8")).hexdigest()[:8]
    return sprite_hash
...
def display_custom_404(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        try: return view_func(request, *args, **kwargs)
        except Http404: return page_not_found(request, '', template_name='cms/404.html')
    return wrapper
