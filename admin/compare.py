...
def text_from_html(val): return BeautifulSoup(force_str(val), 'html5lib').getText()
...
class FieldComparison:
    ...
    def field_label(self):
        ...
        if verbose_name is None: verbose_name = self.field.name.replace('_', ' ')
        return capfirst(verbose_name)
    def htmldiff(self):
        if self.val_a != self.val_b: return TextDiff([('deletion', self.val_a), ('addition', self.val_b)]).to_html()
        else: return escape(self.val_a)
    def has_changed(self): return self.val_a != self.val_b

class TextFieldComparison(FieldComparison):
    def htmldiff(self): return diff_text(self.val_a, self.val_b).to_html()

class RichTextFieldComparison(TextFieldComparison):
    def htmldiff(self): return diff_text(text_from_html(self.val_a),text_from_html(self.val_b)).to_html()

def get_comparison_class_for_block(block):
    # from streams.custom_fields import RawHTMLBlock
    if hasattr(block, 'get_comparison_class'): return block.get_comparison_class()
    elif isinstance(block, (blocks.CharBlock, blocks.TextBlock)): return CharBlockComparison
    elif isinstance(block, blocks.StructBlock): return StructBlockComparison
    elif isinstance(block, blocks.StreamBlock): return StreamBlockComparison
    elif isinstance(block, blocks.RichTextBlock): return RichTextBlockComparison
    # As all stream field blocks have an HTML representation, fall back to diffing that.
    else: return RichTextBlockComparison
...
class BlockComparison:
    def __init__(self, block, exists_a, exists_b, val_a, val_b):
        ...
        self.val_a = val_a
        self.val_b = val_b
    def is_new(self): return self.exists_b and not self.exists_a
    def is_deleted(self): return self.exists_a and not self.exists_b
    def has_changed(self): return self.val_a != self.val_b
    def htmlvalue(self, val):
        """ Return an HTML representation of this block that is safe to be included in comparison views """
    ...
    def htmldiff(self):
        ...
        return diff_text(text_from_html(html_val_a),text_from_html(html_val_b)).to_html()

class CharBlockComparison(BlockComparison):
    def htmldiff(self): return diff_text(force_str(self.val_a),force_str(self.val_b)).to_html() 
    def htmlvalue(self, val): return escape(val) 

class RichTextBlockComparison(BlockComparison): pass
