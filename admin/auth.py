...
def permission_denied(request):
    ...
    messages.error(request, "Sorry, you do not have permission to access this area.")
...
class PermissionPolicyChecker:
    """ Provides a view decorator that enforces the given permission policy, returning the wagtailadmin 'permission denied' response if permission not granted """
    def __init__(self, policy): self.policy = policy
    def require(self, action):
        def test(user): return self.policy.user_has_permission(user, action)
        return user_passes_test(test)
    def require_any(self, *actions):
        def test(user): return self.policy.user_has_any_permission(user, actions)
        return user_passes_test(test)
...
def user_has_any_page_permission(user):
    ...
    if GroupPagePermission.objects.filter(group__in=user.groups.all()).exists(): return True
    return False


def reject_request(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest': raise PermissionDenied
    from django.contrib.auth.views import redirect_to_login as auth_redirect_to_login
    return auth_redirect_to_login(request.get_full_path(), login_url=reverse('wagtailadmin_login'))


def require_admin_access(view_func):
    def decorated_view(request, *args, **kwargs):
        user = request.user
        if user.is_anonymous: return reject_request(request)
            ...
            try:
                ...
                with LogContext(user=user):
                    if preferred_language:
                        with override(preferred_language): response = view_func(request, *args, **kwargs)
                        if hasattr(response, "render"):
                            # If the response has a render() method, Django treats it like a TemplateResponse, so we should do the same In this case, we need to guarantee that when the TemplateResponse
                            # is rendered, it is done within the override context manager or the user preferred_language will not be used
                            # (this could be replaced with simply rendering the TemplateResponse for simplicity but this does remove some of its middleware modification potential)
                            render = response.render
                            def overridden_render(response):
                                with override(preferred_language): return render()
                ...
                return permission_denied(request)
        if not request.headers.get('x-requested-with') == 'XMLHttpRequest': messages.error(request, "You do not have permission to access this area")
