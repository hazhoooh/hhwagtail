...
class SelectWithDisabledOptions(forms.Select):
    ...
    def create_option(self, name, value, *args, **kwargs):
        ...
        if value in self.disabled_values: option_dict['attrs']['disabled'] = 'disabled'
        return option_dict
...
class CollectionChoiceField(forms.ModelChoiceField):
    ...
    def _get_disabled_queryset(self): return self._disabled_queryset
    def _set_disabled_queryset(self, queryset):
        self._disabled_queryset = queryset
        if queryset is None: self.widget.disabled_values = ()
        else: self.widget.disabled_values = queryset.values_list(self.to_field_name or 'pk', flat=True)
    ...
    def _set_queryset(self, queryset):
        ...
        if min_depth is None: self._indentation_start_depth = 2
        else: self._indentation_start_depth = min_depth + 1
    def label_from_instance(self, obj): return obj.get_indented_name(self._indentation_start_depth, html=True)

class CollectionForm(forms.ModelForm):
    parent = CollectionChoiceField(label="Parent",queryset=Collection.objects.all(),required=False,help_text="Select hierarchical position. Note: a collection cannot become a child of itself or one of its descendants.")
    class Meta:
        model = Collection
        fields = ('name',)
    def clean_parent(self):
        ...
        if not self.instance._state.adding and not parent.pk == self.initial.get('parent'):
            old_descendants = list(self.instance.get_descendants(inclusive=True).values_list('pk', flat=True))
            if parent.pk in old_descendants: raise ValidationError(gettext_lazy('Please select another parent'))
        return parent
...
class BaseCollectionMemberForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if user is None: self.collections = Collection.objects.all()
        else: self.collections = (self.permission_policy.collections_user_has_permission_for(user, 'add'))
        if self.instance.pk:
            # editing an existing document; ensure that the list of available collections
            # includes its current collection
            self.collections = (self.collections | Collection.objects.filter(id=self.instance.collection_id))
        if len(self.collections) == 0: raise Exception("Cannot construct %s for a user with no collection permissions" % type(self))
        elif len(self.collections) == 1: del self.fields['collection']
        else: self.fields['collection'].queryset = self.collections
    def save(self, commit=True):
        if len(self.collections) == 1: self.instance.collection = self.collections[0]
...
def collection_member_permission_formset_factory(model, permission_types, template, default_prefix=None):
    permission_queryset = Permission.objects.filter(content_type__app_label=model._meta.app_label,codename__in=[codename for codename, short_label, long_label in permission_types]).select_related('content_type')
    if default_prefix is None: default_prefix = '%s_permissions' % model._meta.model_name
    class PermissionMultipleChoiceField(forms.ModelMultipleChoiceField):
        """ Allows the custom labels from ``permission_types`` to be applied to permission checkboxes for the ``CollectionMemberPermissionsForm`` below """
        def label_from_instance(self, obj):
            for codename, short_label, long_label in permission_types:
                if codename == obj.codename: return long_label
            return str(obj)
    class CollectionMemberPermissionsForm(forms.Form):
        ...
        """
        collection = CollectionChoiceField(label="Collection",queryset=Collection.objects.all().prefetch_related('group_permissions'),empty_label=None)
        permissions = PermissionMultipleChoiceField(queryset=permission_queryset,required=False,widget=forms.CheckboxSelectMultiple)
    GroupCollectionMemberPermissionFormSet = type(str('GroupCollectionMemberPermissionFormSet'),(BaseGroupCollectionMemberPermissionFormSet, ),{'permission_types': permission_types,'permission_queryset': permission_queryset,'default_prefix': default_prefix,'template': template,})
    return forms.formset_factory(CollectionMemberPermissionsForm,formset=GroupCollectionMemberPermissionFormSet,extra=0,can_delete=True)

GroupCollectionManagementPermissionFormSet = collection_member_permission_formset_factory(Collection, [('add_collection', "Add", "Add collections"),('change_collection', "Edit", "Edit collections"),('delete_collection', "Delete", "Delete collections"),], 'cms/permissions/includes/collection_management_permissions_form.html')
