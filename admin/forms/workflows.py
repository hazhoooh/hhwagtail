...
class TaskChooserSearchForm(forms.Form):
    q = forms.CharField(label="Search term", widget=forms.TextInput(), required=False)
    ...
    def clean(self):
        ...
        try:
            ...
            if not self.errors and existing_workflow != self.cleaned_data['workflow']:
                ...
                self.add_error('page', ValidationError("This page already has workflow '{0}' assigned.".format(existing_workflow), code='existing_workflow'))
...
class BaseWorkflowPagesFormSet(forms.BaseInlineFormSet):
    ...
    def clean(self):
        ...
        if len(set(pages)) != len(pages):
            ...
            raise forms.ValidationError("You cannot assign this workflow to the same page multiple times.")
...
def get_workflow_edit_handler():
    ...
    panels = [
        FieldPanel("name", heading="Give your workflow a name", classname="full title"),
        InlinePanel("workflow_tasks", [
            FieldPanel('task', widget=AdminTaskChooser(show_clear_link=False)),
        ], heading="Add tasks to your workflow"),
    ]
