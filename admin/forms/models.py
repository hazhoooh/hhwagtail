...
# DIRECT_FORM_FIELD_OVERRIDES = {models.TextField: {'widget': widgets.AdminAutoHeightTextInput},}
...
def formfield_for_dbfield(db_field, **kwargs):
    ...
    # if overrides:
    #     if callable(overrides): overrides = overrides(db_field)
...
class WagtailAdminModelFormMetaclass(ClusterFormMetaclass):
    ...
    # which handles the representation of model fields in form fields -
    # to use our own formfield_for_dbfield function to do that representation.
    ...
    def __new__(cls, name, bases, attrs):
        if 'formfield_callback' not in attrs or attrs['formfield_callback'] is None: attrs['formfield_callback'] = formfield_for_dbfield
        new_class = super(WagtailAdminModelFormMetaclass, cls).__new__(cls, name, bases, attrs)
        return new_class
    @classmethod
    def child_form(cls): return WagtailAdminModelForm
...
class WagtailAdminModelForm(ClusterForm, metaclass=WagtailAdminModelFormMetaclass):
    ...
    def media(self):
        ...
        media = super().media
        for formset in self.formsets.values(): media += formset.media
        return media
