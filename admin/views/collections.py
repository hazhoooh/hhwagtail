...
class Index(IndexView):
    ...
    template_name = 'cms/collections/index.html'
    ...
    def get_queryset(self): return self.permission_policy.instances_user_has_any_permission_for(self.request.user, ['add', 'change', 'delete']).exclude(depth=1)
...
class Edit(EditView):
    ...
    model = Collection
    form_class = CollectionForm
    template_name = 'cms/collections/edit.html'
    ...
    def _user_may_move_collection(self, user, instance):
        """ Is this instance used for assigning GroupCollectionPermissions to the user? If so, this user is not allowed do move the collection to a new part of the tree """
        if user.is_active and user.is_superuser: return True
        else: return not GroupCollectionPermission.objects.filter(group__user=user,permission__in=self.permission_policy._get_permission_objects_for_actions(['add', 'edit', 'delete']),collection=instance,).exists()
    def get_queryset(self): return self.permission_policy.instances_user_has_permission_for(self.request.user, 'change').exclude(depth=1)
    def get_form(self, form_class=None):
        form = super().get_form(form_class)
        ...
        elif not self._user_may_move_collection(user, form.instance): form.fields.pop('parent')
        else:
    ...
    def save_instance(self):
        ...
        if 'parent' in self.form.changed_data: instance.move(self.form.cleaned_data['parent'], 'sorted-child')
        return instance
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['can_delete'] = self.permission_policy.instances_user_has_permission_for(self.request.user, 'delete').filter(pk=self.object.pk).first()
        return context
...
class Delete(DeleteView):
    ...
    def get_queryset(self):
        return self.permission_policy.instances_user_has_permission_for(self.request.user, 'delete').exclude(depth=1)
    def get_collection_contents(self):
        collection_contents = [hook(self.object) for hook in hooks.get_hooks('describe_collection_contents')]
    ...
    def get_context_data(self, **kwargs):
        ...
        if collection_contents:
            self.template_name = 'cms/collections/delete_not_empty.html' # collection is non-empty; render the 'not allowed to delete' response
            context['collection_contents'] = collection_contents
        return context
    def post(self, request, pk):
        self.object = get_object_or_404(self.get_queryset(), id=pk)
        collection_contents = self.get_collection_contents()
        if collection_contents: return HttpResponseForbidden() # collection is non-empty; refuse to delete it
        self.object.delete()
