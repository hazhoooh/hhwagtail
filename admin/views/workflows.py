...
from wagtail.admin.forms.workflows import TaskChooserSearchForm, WorkflowPagesFormSet, get_task_form_class, get_workflow_edit_handler
from wagtail.admin.modal_workflow import render_modal_workflow
from wagtail.admin.views.generic import CreateView, DeleteView, EditView, IndexView
from wagtail.core.models import Page, Task, TaskState, UserPagePermissionsProxy, Workflow, WorkflowState
...
class Create(CreateView):
    ...
    template_name = 'cms/workflows/create.html'
    success_message = "Workflow '{0}' created."
    add_url_name = 'wagtailadmin_workflows:add'
    edit_url_name = 'wagtailadmin_workflows:edit'
    index_url_name = 'wagtailadmin_workflows:index'
    header_icon = '✅'
            ...
            if pages_formset.is_valid():
                ...
                if success_message is not None:
                    messages.success(self.request, success_message, buttons=[
                        messages.button(reverse(self.edit_url_name, args=(self.object.id,)), "Edit")
                    ])
...
class Edit(EditView):
    permission_policy = workflow_permission_policy
    model = Workflow
    page_title = "Editing workflow"
    template_name = 'cms/workflows/edit.html'
    success_message = "Workflow '{0}' updated."
    add_url_name = 'wagtailadmin_workflows:add'
    edit_url_name = 'wagtailadmin_workflows:edit'
    delete_url_name = 'wagtailadmin_workflows:disable'
    delete_item_label = "Disable"
    index_url_name = 'wagtailadmin_workflows:index'
    enable_item_label = "Enable"
...
class Disable(DeleteView):
    permission_policy = workflow_permission_policy
    model = Workflow
    page_title = "Disable workflow"
    template_name = 'cms/workflows/confirm_disable.html'
    success_message = "Workflow '{0}' disabled."
    add_url_name = 'wagtailadmin_workflows:add'
    edit_url_name = 'wagtailadmin_workflows:edit'
    delete_url_name = 'wagtailadmin_workflows:disable'
    index_url_name = 'wagtailadmin_workflows:index'
...
def usage(request, pk):
    ...
    paginator = Paginator(pages, per_page=150)
    pages = paginator.get_page(request.GET.get('p'))

    return render(request, 'cms/workflows/usage.html', {
        'workflow': workflow,
        'used_by': pages,
    })
...
def enable_workflow(request, pk):
    ...
    if not workflow.active:
        ...
        workflow.save()
        messages.success(request, "Workflow '{0}' enabled.".format(workflow.name))
...
def remove_workflow(request, page_pk, workflow_pk=None):
    ...
    if hasattr(page, 'workflowpage'):
        ...
        if not workflow_pk or workflow_pk == page.workflowpage.workflow.pk:
            ...
            messages.success(request, "Workflow removed from Page '{0}'.".format(page.get_admin_display_title()))
...
class TaskIndex(IndexView):
    ...
    template_name = 'cms/workflows/task_index.html'
    ...
    add_item_label = "New workflow task"
    header_icon = '📌'
...
def select_task_type(request):
    ...
    return render(request, 'cms/workflows/select_task_type.html', {
...
class CreateTask(CreateView):
    permission_policy = task_permission_policy
    model = None
    page_title = "New workflow task"
    template_name = 'cms/workflows/create_task.html'
    success_message = "Task '{0}' created."
...
class EditTask(EditView):
    permission_policy = task_permission_policy
    model = None
    page_title = "Editing workflow task"
    template_name = 'cms/workflows/edit_task.html'
    success_message = "Task '{0}' updated."
    add_url_name = 'wagtailadmin_workflows:select_task_type'
    edit_url_name = 'wagtailadmin_workflows:edit_task'
    ...
    @cached_property
    def page_title(self):
        return "Editing %(task_type)s" % {'task_type': self.get_object().content_type.name}
...
class DisableTask(DeleteView):
    permission_policy = task_permission_policy
    model = Task
    page_title = "Disable task"
    template_name = 'cms/workflows/confirm_disable_task.html'
    success_message = "Task '{0}' disabled."
    add_url_name = 'wagtailadmin_workflows:add_task'
    edit_url_name = 'wagtailadmin_workflows:edit_task'
    delete_url_name = 'wagtailadmin_workflows:disable_task'
    index_url_name = 'wagtailadmin_workflows:task_index'
...
def enable_task(request, pk):
    ...
    if not task.active:
        task.active = True
        task.save()
        messages.success(request, "Task '{0}' enabled.".format(task.name))
...
def task_chooser(request):
    ...
    if 'q' in request.GET or 'p' in request.GET or 'task_type' in request.GET:
        ...
        # Pagination
        paginator = Paginator(tasks, per_page=50)
        tasks = paginator.get_page(request.GET.get('p'))

        return TemplateResponse(request, "cms/workflows/task_chooser/includes/results.html", {
            'task_types': task_types,
        ...
        return render_modal_workflow(request, 'cms/workflows/task_chooser/chooser.html', None, {
            'task_types': task_types,
            'tasks': tasks,
            'searchform': searchform,
