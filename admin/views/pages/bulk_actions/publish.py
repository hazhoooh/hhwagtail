...
class PublishBulkAction(PageBulkAction):
    ...
    action_type = "publish"
    aria_label = "Publish selected pages"
    template_name = "cms/pages/bulk_actions/confirm_bulk_publish.html"
    action_priority = 40
        ...
        if num_parent_objects == 1:
            if include_descendants:
                if num_child_objects == 0:
                    success_message = "1 page has been published"
                else:
            ...
            if include_descendants:
                if num_child_objects == 0:
                    success_message = "%(num_parent_objects)d pages have been published" % {'num_parent_objects': num_parent_objects}
                else:
                    success_message = ngettext(
