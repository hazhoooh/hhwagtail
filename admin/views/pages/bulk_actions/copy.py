# TODO https://docs.wagtail.org/en/stable/extending/custom_bulk_actions.html   for copying multiple page objects
...
class MoveForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['chooser'] = forms.ModelChoiceField(
            ...
            label="Select a new parent page",
        )
...
class MoveBulkAction(PageBulkAction):
    display_name = "Move"
    action_type = "move"
    aria_label = "Move selected pages"
    template_name = "cms/pages/bulk_actions/confirm_bulk_move.html"
    action_priority = 10
    form_class = MoveForm
    destination = None
    ...
    def get_actionable_objects(self):
        ...
        for page in objects:
            if not page.permissions_for_user(request.user).can_move_to(destination) and not request.user.is_superuser:
