...
class UnpublishBulkAction(PageBulkAction):
    display_name = "Unpublish"
    action_type = "unpublish"
    aria_label = "Unpublish selected pages"
    template_name = "cms/pages/bulk_actions/confirm_bulk_unpublish.html"
    action_priority = 50
        ...
        if num_parent_objects == 1:
            if include_descendants:
                if num_child_objects == 0:
                    success_message = "1 page has been unpublished"
                else:
            ...
            if include_descendants:
                if num_child_objects == 0:
                    success_message = "%(num_parent_objects)d pages have been unpublished" % {'num_parent_objects': num_parent_objects}
                else:
                    success_message = ngettext(
