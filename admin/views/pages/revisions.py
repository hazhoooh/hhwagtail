...
def revisions_index(request, page_id):
    ...
    paginator = Paginator(revisions, per_page=150)
    revisions = paginator.get_page(request.GET.get('p'))

    return TemplateResponse(request, 'cms/pages/revisions/index.html', {
        'page': page,
        'ordering': ordering,
...
def revisions_revert(request, page_id, revision_id):
    ...
    user_avatar = render_to_string('cms/shared/user_avatar.html', {'user': revision.user})
    ...
    return TemplateResponse(request, 'cms/pages/edit.html', {
        'page': page,
        'revision': revision,
        'is_revision': True,
...
def revisions_compare(request, page_id, revision_id_a, revision_id_b):
    ...
    return TemplateResponse(request, 'cms/pages/revisions/compare.html', {
        'page': page,
...
def revisions_unschedule(request, page_id, revision_id):
    ...
    subtitle = "revision {0} of '\{1\}'".format(revision.id, page.get_admin_display_title())

    if request.method == 'POST':
        revision.approved_go_live_at = None
        revision.save(user=request.user, update_fields=['approved_go_live_at'])

        messages.success(request, "Version {0} of '{1}' unscheduled.".format(revision.id, page.get_admin_display_title()), buttons=[
    ...
    return TemplateResponse(request, 'cms/pages/revisions/confirm_unschedule.html', {
        'page': page,
        'revision': revision,
        'next': next_url,
        'subtitle': subtitle
