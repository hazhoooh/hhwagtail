...
def view_draft(request, page_id):
    ...
    if not (perms.can_publish() or perms.can_edit()): raise PermissionDenied
    try: preview_mode = page.default_preview_mode
    except IndexError: raise PermissionDenied
...
class PreviewOnEdit(View):
    ...
    def remove_old_preview_data(self):
        ...
        expired_keys = [k for k, v in self.request.session.items() if k.startswith(self.session_key_prefix) and v[1] < expiration]
        # Removes the session key gracefully
        for k in expired_keys: self.request.session.pop(k)
    @property
    def session_key(self): return f"{self.session_key_prefix}{self.kwargs['page_id']}"
    def get_page(self): return get_object_or_404(Page,id=self.kwargs["page_id"]).get_latest_revision_as_page()
    ...
    def error_response(self, page): return TemplateResponse(self.request, 'cms/pages/preview_error.html',{'page': page})
    def get(self, request, *args, **kwargs):
        page = self.get_page()
        post_data, timestamp = self.request.session.get(self.session_key, (None, None))
        if not isinstance(post_data, str): post_data = ''
        form = self.get_form(page, QueryDict(post_data))
        if not form.is_valid(): return self.error_response(page)
        form.save(commit=False)
        try: preview_mode = request.GET.get('mode', page.default_preview_mode)
        except IndexError: raise PermissionDenied
        return page.make_preview_request(request, preview_mode)

class PreviewOnCreate(PreviewOnEdit):
    @property
    def session_key(self): return f"{self.session_key_prefix}{self.kwargs['content_type_app_name']}-{self.kwargs['content_type_model_name']}-{self.kwargs['parent_page_id']}"
    def get_page(self):
        ...
        try: content_type = ContentType.objects.get_by_natural_key(content_type_app_name, content_type_model_name)
        except ContentType.DoesNotExist: raise Http404
        page = content_type.model_class()()
        parent_page = get_object_or_404(Page, id=parent_page_id).specific
        page.depth = parent_page.depth + 1
        if parent_page.is_leaf(): page.path = page._get_path(parent_page.path, page.depth, 1)
        else: page.path = parent_page.get_last_child()._inc_path()
        return page
    def get_form(self, page, query_dict):
