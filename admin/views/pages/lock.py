...
def lock(request, page_id):
    ...
    if not page.locked:
        ...
        page.save(user=request.user, log_action='locked')
...
def unlock(request, page_id):
    ...
    if page.locked:
        page.locked = False
        page.save(user=request.user, log_action='unlocked')

        messages.success(request, "Page '{0}' is now unlocked.".format(page.get_admin_display_title()), extra_tags='unlock')
