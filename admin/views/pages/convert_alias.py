...
def convert_alias(request, page_id):
    ...
    if not page.permissions_for_user(request.user).can_edit(): raise PermissionDenied
    with transaction.atomic():
        ...
        if request.method == 'POST':
            ...
            PageLogEntry.objects.log_action(
                instance=page,
                revision=revision,
                action='alias_converted',
                user=request.user,
                data={'page': {'id': page.id,'title': page.get_admin_display_title()},},)
            messages.success(request, "Page '{0}' has been converted into an ordinary page.".format(page.get_admin_display_title()))
            for fn in hooks.get_hooks('after_convert_alias_page'):
                result = fn(request, page)
                if hasattr(result, 'status_code'): return result
            if next_url: return redirect(next_url)
            return redirect('wagtailadmin_pages:edit', page.id)
    return TemplateResponse(request, 'cms/pages/confirm_convert_alias.html', {'page': page,'next': next_url,})
