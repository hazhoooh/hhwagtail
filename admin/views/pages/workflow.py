...
class BaseWorkflowFormView(View):
    ...
    def dispatch(self, request, page_id, action_name, task_state_id):
        ...
        if not self.page.workflow_in_progress:
            messages.error(request, "The page '{0}' is not currently awaiting moderation.".format(self.page.get_admin_display_title()))
    ...
    def render_modal_form(self, request, form):
        return render_modal_workflow(
            request, 'cms/pages/workflow_action_modal.html', None, {
                'page': self.page,
                'form': form,
...
def confirm_workflow_cancellation(request, page_id):
    ...
    if (not workflow_state) or not getattr(settings, 'CONSOLE_WORKFLOW_CANCEL_ON_PUBLISH', True):
        return render_modal_workflow(request, '', None, {}, json_data={'step': 'no_confirmation_needed'})

    return render_modal_workflow(
        request, 'cms/pages/confirm_workflow_cancellation.html', None, {
...
def workflow_status(request, page_id):
    ...
    return render_modal_workflow(request, 'cms/workflows/workflow_status.html', None, {
        'page': page,
...
def preview_revision_for_task(request, page_id, task_id):
    ...
    try:
        ...
        messages.error(request, "The page '{0}' is not currently awaiting moderation in task '{1}'.".format(page.get_admin_display_title(), task.name))
