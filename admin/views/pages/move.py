...
def move_choose_destination(request, page_to_move_id, viewed_page_id=None):
    ...
    return TemplateResponse(request, 'cms/pages/move_choose_destination.html', {
...
def move_confirm(request, page_to_move_id, destination_id):
    ...
    if not Page._slug_is_available(page_to_move.slug, destination, page=page_to_move):
        messages.error(
            request,
            "The slug '{0}' is already in use at the selected parent page. Make sure the slug is unique and try again".format(page_to_move.slug)
        )
    ...
    if request.method == 'POST':
        ...
        messages.success(request, "Page '{0}' moved.".format(page_to_move.get_admin_display_title()), buttons=[
            messages.button(reverse('wagtailadmin_pages:edit', args=(page_to_move.id,)), "Edit")
        ])
    ...
    return TemplateResponse(request, 'cms/pages/confirm_move.html', {
        'page_to_move': page_to_move,
