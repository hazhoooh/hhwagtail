...
def set_privacy(request, page_id):
    ...
    if not page_perms.can_publish(): raise PermissionDenied
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and not restriction_exists_on_ancestor:
            if form.cleaned_data['restriction_type'] == PageViewRestriction.NONE:
                if restriction: restriction.delete(user=request.user)
            else:
        ...
        if not restriction_exists_on_ancestor:
            if restriction: form = PageViewRestrictionForm(instance=restriction)
            else: form = PageViewRestrictionForm(initial={'restriction_type': 'none'})
    if restriction_exists_on_ancestor: return render_modal_workflow(request, 'cms/pages/ancestor_privacy.html', None,{'page_with_restriction': restriction.page,})
    else: return render_modal_workflow(request, 'cms/pages/set_privacy.html', None, {'page': page,'form': form,}, json_data={'step': 'set_privacy'})
