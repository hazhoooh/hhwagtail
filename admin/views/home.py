...
from wagtail.admin.navigation import get_site_for_user, get_sites_for_user
from wagtail.admin.site_summary import SiteSummaryPanel
from wagtail.admin.ui.components import Component
from wagtail.core import hooks
from wagtail.core.models import Page, PageRevision, TaskState, UserPagePermissionsProxy, WorkflowState
from wagtail.utils.deprecation import RemovedInWagtail217Warning

User = get_user_model()

# Panels for the console
class PagesForModerationPanel(Component):
    name = 'pages_for_moderation'
    template_name = 'cms/console_boards/pages_for_moderation.html'
    order = 200
...
class UserPagesInWorkflowModerationPanel(Component):
    ...
    template_name = 'cms/console_boards/user_pages_in_workflow_moderation.html'
    order = 210
    def get_context_data(self, parent_context):
        request = parent_context['request']
        context = super().get_context_data(parent_context)
...
class WorkflowPagesToModeratePanel(Component):
    ...
    template_name = 'cms/console_boards/workflow_pages_to_moderate.html'
    order = 220
    def get_context_data(self, parent_context):
        request = parent_context['request']
        context = super().get_context_data(parent_context)
...
class LockedPagesPanel(Component):
    name = 'locked_pages'
    template_name = 'cms/console_boards/locked_pages.html'
    order = 300
    def get_context_data(self, parent_context):
        request = parent_context['request']
        context = super().get_context_data(parent_context)
...
class RecentEditsPanel(Component):
    name = 'recent_edits'
    template_name = 'cms/console_boards/recent_edits.html'
    order = 250
    def get_context_data(self, parent_context):
        request = parent_context['request']
        context = super().get_context_data(parent_context)
        ...
        # Last n edited pages
        edit_count = getattr(settings, 'CONSOLE_RECENT_EDITS_LIMIT', 5)
...
def home(request):
    ...
    for panel in panels:
        if hasattr(panel, 'render') and not hasattr(panel, 'render_html'):
            ...
            # cms/console.html should be removed too
            message = (
    ...
    u_site = get_site_for_user(request.user)
    u_sites = get_sites_for_user(request.user)

    return TemplateResponse(request, "console/home.html", {
        'user_site': u_site,
        'user_sites': u_sites,
...
def icons():
    ...
    if _icons_html is None:
        ...
        _icons_html = render_to_string("cms/shared/icons.html", {'icons': all_icons})
    return _icons_html
