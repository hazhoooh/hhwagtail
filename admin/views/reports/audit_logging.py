...
class SiteHistoryReportFilterSet(WagtailFilterSet):
    ...
    timestamp = django_filters.DateTimeFromToRangeFilter(label="Date")
    label = django_filters.CharFilter(label="Name", lookup_expr='icontains')
    user = django_filters.ModelChoiceFilter(field_name='user', queryset=lambda request: get_users_for_filter())
    object_type = ContentTypeFilter(label="Type",method='filter_object_type', queryset=lambda request: get_content_types_for_filter())
    def filter_object_type(self, queryset, name, value): return queryset.filter_on_content_type(value)
    class Meta:
        model = PageLogEntry
        fields = ['object_type', 'label', 'action', 'user', 'timestamp']

class LogEntriesView(ReportView):
    template_name = 'cms/reports/site_history.html'
    title = "Site history"
    header_icon = ''
    filterset_class = SiteHistoryReportFilterSet
    export_headings = {"object_id": "ID","label": "Name","content_type": "Type","action": "Action type","user_display_name": "User","timestamp": "Date/Time"}
    ...
    def get_filename(self): return "audit-log-{}".format(datetime.datetime.today().strftime("%Y-%m-%d"))
    def get_filtered_queryset(self):
        """
            Since this report combines records from multiple log models, the standard pattern of returning a queryset from get_queryset() to be filtered by filter_queryset() is not possible - the subquery for each log model must be filtered separately before joining with union().
            Additionally, a union() on standard model-based querysets will return a queryset based on the first model in the union, so instances of the other model(s) would be returned as the wrong type.
            To avoid this, we construct values() querysets as follows:
                1. For each model, construct a values() queryset consisting of id, timestamp and an annotation to indicate which model it is, and filter this with filter_queryset
                2. Form a union() queryset from these queries, and order it by -timestamp (this is the result returned from get_filtered_queryset)
                3. Apply pagination (done in MultipleObjectMixin.get_context_data)
                4. (In decorate_paginated_queryset:) For each model included in the result set, look up the set of model instances by ID. Use these to form a final list of model instances in the same order as the query.
        """
        queryset = None
        filters = None
        # Retrieve the set of registered log models, and cast it to a list so that we assign an index number to each one; this index number will be used to distinguish models in the combined results
        ...
        for log_model_index, log_model in enumerate(self.log_models):
            ...
            if queryset is None: queryset = sub_queryset
            else: queryset = queryset.union(sub_queryset)
    ...
    def decorate_paginated_queryset(self, queryset):
        ...
        for row in queryset: pks_by_log_model_index[row['log_model_index']].append(row['pk'])
        url_finder = AdminURLFinder(self.request.user)
        # for each log model found in the queryset, look up the set of log entries by id and build a lookup table
        object_lookup = {}
    ...
    def get_action_label(self, action): return force_str(log_action_registry.get_action_label(action))
