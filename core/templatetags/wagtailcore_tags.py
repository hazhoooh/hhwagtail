...
@register.simple_tag(takes_context=True)
def collections_list_names(context):
    if context.request.user.is_superuser:
        from wagtail.core.models import Collection
        return Collection.objects.all()
    else:
        from wagtail.core.permissions import collection_permission_policy
        return collection_permission_policy.instances_user_has_any_permission_for(context.request.user, ['add', 'change', 'delete']).exclude(depth=1)
