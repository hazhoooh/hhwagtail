...
class TreebeardPathFixMixin:
        ...
        if fix_paths:
            with transaction.atomic():
                ...
                while children_to_fix:
                    ...
                    children = cls.objects.filter(path__startswith=parent_path, depth=depth).values('pk', 'path', 'depth', 'numchild')
                    ...
                    for item in desired_sequence:
                        ...
                        if max_position is None or actual_position > max_position: max_position = actual_position
                    ...
                    for (i, item) in enumerate(desired_sequence):
                        ...
                        actual_position = cls._str2int(item['path'][-cls.steplen:])
                        if actual_position == desired_position: pass
                        else:
    ...
    def _rewrite_node_path(cls, old_path, new_path):
        cls.objects.filter(path__startswith=old_path).update(path=Concat(Value(new_path),Substr('path', len(old_path) + 1)))
