...
class StructBlockValidationErrorAdapter(Adapter):
    ...
    def js_args(self, error):
        if error.block_errors is None: return [None]
        else: return [{name: error_list.as_data() for name, error_list in error.block_errors.items()}]
...
class StructValue(collections.OrderedDict):
    ...
    def __init__(self, block, *args):
        super().__init__(*args)
        self.block = block
    def __html__(self): return self.block.render(self)
    def render_as_block(self, context=None): return self.block.render(self, context=context)
    @cached_property
    def bound_blocks(self): return collections.OrderedDict([(name, block.bind(self.get(name))) for name, block in self.block.child_blocks.items()])
...
class PlaceholderBoundBlock(BoundBlock):
    ...
    def render_form(self):
        return format_html('<div data-structblock-child="{}"></div>', self.block.name) # TODO : check where the data-structblock-child been used, and if possible, reduce the variable characters
...
class BaseStructBlock(Block):
    ...
    def value_from_datadict(self, data, files, prefix): return self._to_struct_value([(name, block.value_from_datadict(data, files, '%s-%s' % (prefix, name))) for name, block in self.child_blocks.items()])
    def value_omitted_from_data(self, data, files, prefix): return all(block.value_omitted_from_data(data, files, '%s-%s' % (prefix, name)) for name, block in self.child_blocks.items())
    ...
    def get_form_context(self, value, prefix='', errors=None):
        return {
            'children': collections.OrderedDict([(name,PlaceholderBoundBlock(block, value.get(name), prefix="%s-%s" % (prefix, name))) for name, block in self.child_blocks.items()]),
...
class StructBlock(BaseStructBlock, metaclass=DeclarativeSubBlocksMetaclass): pass
...
class StructBlockAdapter(Adapter):
    ...
    def js_args(self, block):
        ...
        if block.meta.form_template: meta['formTemplate'] = block.render_form_template()
        if block.meta.label_format: meta['labelFormat'] = block.meta.label_format
