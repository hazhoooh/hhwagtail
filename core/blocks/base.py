...
from wagtail.core.utils import blocks_static
...
class Block(metaclass=BaseBlock):
    ...
    def __init__(self, **kwargs):
        if 'classname' in self._constructor_args[1]:
            # Adding this so that migrations are not triggered when form_classname is used instead of classname in the initialisation of the FieldBlock
        ...
        for attr, value in kwargs.items(): setattr(self.meta, attr, value)
    ...
    def set_name(self, name):
        self.name = name
        if not self.meta.label: self.label = capfirst(force_str(name).replace('_', ' '))
    def set_meta_options(self, opts):
        """ Update this block's meta options (out of the ones designated as mutable) from the given dict. Used by the StreamField constructor to pass on kwargs that are to be handled by the block, since the block object has already been created by that point, e.g.: body = StreamField(SomeStreamBlock(), max_num=5) """
        for attr, value in opts.items():
            if attr in self.MUTABLE_META_ATTRIBUTES: setattr(self.meta, attr, value)
            else: raise TypeError("set_meta_options received unexpected option: %r" % attr)
...
class BoundBlock:
    def __init__(self, block, value, prefix=None, errors=None):
        ...
        self.prefix = prefix
        self.errors = errors
    def render(self, context=None): return self.block.render(self.value, context=context)
    def render_as_block(self, context=None):
        """
        Alias for render; the include_block tag will specifically check for the presence of a method with this name. (This is because {% include_block %} is just as likely to be invoked on a bare
        value as a BoundBlock. If we looked for a `render` method instead, we'd run the risk of finding an unrelated method that just happened to have that name - for example, when called on a
        PageChooserBlock it could end up calling page.render.
        """
        return self.block.render(self.value, context=context)
    def id_for_label(self): return self.block.id_for_label(self.prefix)
    def __str__(self): return self.block.render(self.value)  # Render the value according to the block's native rendering
    def __repr__(self): return "<block %s: %r>" % (self.block.name or type(self.block).__name__, self.value)

class DeclarativeSubBlocksMetaclass(BaseBlock):
    """ Metaclass that collects sub-blocks declared on the base classes. """
    def __new__(mcs, name, bases, attrs):
        # Collect sub-blocks declared on the current class. These are available on the class as `declared_blocks`
        current_blocks = []
        ...
        # Walk through the MRO, collecting all inherited sub-blocks, to make the combined `base_blocks`.
        base_blocks = collections.OrderedDict()
        for base in reversed(new_class.__mro__):
            # Collect sub-blocks from base class.
            if hasattr(base, 'declared_blocks'): base_blocks.update(base.declared_blocks)
            # Field shadowing.
            for attr, value in base.__dict__.items():
                if value is None and attr in base_blocks: base_blocks.pop(attr)
...
class BlockWidget(forms.Widget):
    ...
    @property
    def js_context(self):
        if self._js_context is None: self._build_block_json()
    ...
    def render_with_errors(self, name, value, attrs=None, errors=None, renderer=None):
        ...
        if errors: errors_json = json.dumps(self.js_context.pack(errors.as_data()))
        else: errors_json = '[]'
        # TODO : check where the data-xxxx been used, and if possible, reduce the variable characters
        return format_html(""" <div id="{id}" data-block="{block_json}" data-value="{value_json}" data-errors="{errors_json}"></div><script>initBlockWidget('{id}');</script> """,id=name, block_json=self.block_json, value_json=value_json, errors_json=errors_json)
    def render(self, name, value, attrs=None, renderer=None): return self.render_with_errors(name, value, attrs=attrs, errors=None, renderer=renderer)
    @cached_property
    def media(self): return self.js_context.media + forms.Media(js=[static('aw/js/telepath/telepath.js')+"?v0", blocks_static(),],css={'all': [static('aw/css/panels/streamfield.css')+"?v15",]})
    def value_from_datadict(self, data, files, name): return self.block_def.value_from_datadict(data, files, name)
    def value_omitted_from_data(self, data, files, name): return self.block_def.value_omitted_from_data(data, files, name)
...
class BlockField(forms.Field):
    ...
    def __init__(self, block=None, **kwargs):
        if block is None: raise ImproperlyConfigured("BlockField was not passed a 'block' object")
        self.block = block 
        if 'widget' not in kwargs: kwargs['widget'] = BlockWidget(block)
        super().__init__(**kwargs)
    def clean(self, value): return self.block.clean(value)
    def has_changed(self, initial_value, data_value): return self.block.get_prep_value(initial_value) != self.block.get_prep_value(data_value)
