...
class FieldBlock(Block):
    ...
    def id_for_label(self, prefix): return self.field.widget.id_for_label(prefix)
    def value_from_form(self, value): return value
    def value_for_form(self, value): return value
    def value_from_datadict(self, data, files, prefix): return self.value_from_form(self.field.widget.value_from_datadict(data, files, prefix))
    def value_omitted_from_data(self, data, files, prefix): return self.field.widget.value_omitted_from_data(data, files, prefix)
    def clean(self, value): return self.value_from_form(self.field.clean(self.value_for_form(value)))
    @property
    def required(self): return self.field.required
    def get_form_state(self, value): return self.field.widget.format_value(self.field.prepare_value(self.value_for_form(value)))
    class Meta:
        icon = ""
        default = None
...
class FieldBlockAdapter(Adapter):
    ...
    def js_args(self, block):
        classname = ['field',camelcase_to_underscore(block.field.__class__.__name__),block.name,]
        form_classname = getattr(block.meta, 'form_classname', '')
        if form_classname: classname.append(form_classname)
        meta = {
            'label': block.label,
        ...
        if block.field.help_text: meta['helpText'] = block.field.help_text
        return [block.name,block.field.widget,meta]

register(FieldBlockAdapter(), FieldBlock)

class JSONBlock(FieldBlock):
    """ JSON field block """
    def __init__(self, required=True, help_text=None, max_length=None, min_length=None, validators=(), **kwargs):
        from wagtail.admin.widgets import JSONWidget
        self.field = forms.CharField(widget=JSONWidget,required=required,help_text=help_text,max_length=max_length,min_length=min_length,validators=validators,)
        super().__init__(**kwargs)

    def clean(self, value):
        try: json.loads(value)
        except json.decoder.JSONDecodeError as e: raise ValidationError(f'Invalid JSON: {e}')
...
class CharBlock(FieldBlock):
    def __init__(self, required=True, help_text=None, max_length=None, min_length=None, validators=(), **kwargs):
        ...
        super().__init__(**kwargs)
    def get_searchable_content(self, value):return [force_str(value)]

class TextBlock(FieldBlock):
    def __init__(self, required=True, help_text=None, rows=4, max_length=None, min_length=None, validators=(), **kwargs):
    ...
    def field(self):
        ...
        field_kwargs = {'widget': widgets.Textarea}
...
class BlockQuoteBlock(TextBlock):
    def render_basic(self, value, context=None):
        if value: return format_html('<blockquote>{0}</blockquote>', value)
        else: return ''
    class Meta: icon = "“"
...
class BooleanBlock(FieldBlock):
    def __init__(self, required=True, help_text=None, **kwargs):
        ...
        super().__init__(**kwargs)
    def get_form_state(self, value): return bool(value)
    class Meta: icon = "✓"
...
class BaseRawHTMLBlock(FieldBlock):
    ...
    def __init__(self, required=True, help_text=None, max_length=None, min_length=None, validators=(), **kwargs):
        ...
        super().__init__(**kwargs)
    def get_default(self): return mark_safe(self.meta.default or '')
    def to_python(self, value): return mark_safe(value)
    def get_prep_value(self, value): return str(value) + '' # explicitly convert to a plain string, just in case we're using some serialisation method that doesn't cope with SafeString values correctly
    def value_for_form(self, value): return str(value) + '' # need to explicitly mark as unsafe, or it'll output unescaped HTML in the textarea
    def value_from_form(self, value): return mark_safe(value)
    class Meta: icon = '{}'
...
class ChooserBlock(FieldBlock):
    ...
    def to_python(self, value):
        if value is None: return value # the incoming serialised value should be None or an ID
        else:
            try: return self.target_model.objects.get(pk=value)
            except self.target_model.DoesNotExist: return None
    ...
    def get_prep_value(self, value):
        ...
        else: return value.pk

    def value_from_form(self, value): # ModelChoiceField sometimes returns an ID, and sometimes an instance; we want the instance
        if value is None or isinstance(value, self.target_model):return value
        else:
            try:return self.target_model.objects.get(pk=value)
            except self.target_model.DoesNotExist:return None
    ...
    def clean(self, value):
        ...
        # around that...
        if isinstance(value, self.target_model): value = value.pk
        return super().clean(value)
...
class PageChooserBlock(ChooserBlock):
    ...
    def get_form_state(self, value):
        ...
        if value_data is None:return None
        else:
            return {
                'id': value_data['id'],
    ...
    def render_basic(self, value, context=None):
        if value:return format_html('<a href="{0}">{1}</a>', value.url, value.title)
        else:return ''
...
block_classes = [
    FieldBlock, CharBlock, URLBlock, RichTextBlock, BaseRawHTMLBlock, ChooserBlock,
    PageChooserBlock, TextBlock, BooleanBlock, EmailBlock,
    IntegerBlock, FloatBlock, DecimalBlock, RegexBlock, BlockQuoteBlock, JSONBlock
]
DECONSTRUCT_ALIASES = {
