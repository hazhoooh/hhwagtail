...
class StreamBlockValidationErrorAdapter(Adapter):
    ...
    @cached_property
    def media(self): return forms.Media(js=[blocks_static(),])
...
class BaseStreamBlock(Block):
    ...
    def clean(self, value):
        ...
        if self.meta.min_num is not None and self.meta.min_num > len(value):
            non_block_errors.append(ValidationError(
                "The minimum number of items is %d" % self.meta.min_num
            ))
        elif self.required and len(value) == 0:
            non_block_errors.append(ValidationError("This field is required."))

        if self.meta.max_num is not None and self.meta.max_num < len(value):
            non_block_errors.append(ValidationError(
                "The maximum number of items is %d" % self.meta.max_num
            ))
        ...
        if self.meta.block_counts:
            ...
            for block_name, min_max in self.meta.block_counts.items():
                ...
                if min_num is not None and min_num > block_count:
                    non_block_errors.append(ValidationError(
                        '{}: {}'.format(block.label, "The minimum number of items is %d" % min_num)
                    ))
                if max_num is not None and max_num < block_count:
                    non_block_errors.append(ValidationError(
                        '{}: {}'.format(block.label, "The maximum number of items is %d" % max_num)
                    ))
    ...
    def get_form_state(self, value):
        if not value: return []
        else: return [{'type': child.block.name,'value': child.block.get_form_state(child.value),'id': child.id,} for child in value]
    def get_api_representation(self, value, context=None):
        if value is None: return []
        return [{'type': child.block.name,'value': child.block.get_api_representation(child.value, context=context),'id': child.id} for child in value]   # child is a StreamChild instance
    def render_basic(self, value, context=None):
        return format_html_join('\n', '{0}',[(child.render(context=context), child.block_type) for child in value])
    def get_searchable_content(self, value):
        content = []
        for child in value: content.extend(child.block.get_searchable_content(child.value))
        return content
...
class StreamBlock(BaseStreamBlock, metaclass=DeclarativeSubBlocksMetaclass): pass
