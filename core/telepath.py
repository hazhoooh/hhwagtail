...
class WagtailJSContextBase(JSContextBase):
    @property
    def base_media(self): return forms.Media(js=[static(self.telepath_js_path),])

class WagtailAdapterRegistry(AdapterRegistry): js_context_base_class = WagtailJSContextBase

registry = WagtailAdapterRegistry(telepath_js_path='aw/js/telepath/telepath.js')
JSContext = registry.js_context_class

def register(adapter, cls): registry.register(adapter, cls)
