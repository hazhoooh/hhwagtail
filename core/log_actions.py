...
class LogFormatter:
    """
        Defines how to format log messages / comments for a particular action type. Messages that depend on log entry data should override format_message / format_comment;
        static messages can just be set as the 'message' / 'comment' attribute to be registered with log_registry.register_action.
    """
    label = ''
    message = ''
    comment = ''
    ...
    def format_comment(self, log_entry): return self.comment
_active = Local()

class LogContext:
    """ Stores data about the environment in which a logged action happens - e.g. the active user - to be stored in the log entry for that action. """
    def __init__(self, user=None, generate_uuid=True):
        self.user = user
        if generate_uuid: self.uuid = uuid.uuid4()
        else: self.uuid = None
    def __enter__(self):
    ...
    def __exit__(self, type, value, traceback):
        if self._old_log_context: activate(self._old_log_context)
        else: deactivate()

empty_log_context = LogContext(generate_uuid=False)

def activate(log_context): _active.value = log_context
def deactivate(): del _active.value
def get_active_log_context(): return getattr(_active, 'value', empty_log_context)

class LogActionRegistry:
    """ A central store for log actions. The expected format for registered log actions: Namespaced action, Action label, Action message (or callable) """
    def __init__(self):
        self.has_scanned_for_actions = False # Has the register_log_actions hook been run for this registry?
        self.formatters = {} # Holds the formatter objects, keyed by action
        self.choices = [] # Holds a list of action, action label tuples for use in filters
        self.log_entry_models_by_type = {} # Tracks which LogEntry model should be used for a given object class
        self.log_entry_models = set() # All distinct log entry models registered with register_model
    def scan_for_actions(self):
        if not self.has_scanned_for_actions:
            for fn in hooks.get_hooks('register_log_actions'): fn(self)
    ...
    def register_action(self, action, *args):
        ...
        if args:
            # register_action has been invoked as register_action(action, label, message); create a LogFormatter subclass and register that
            label, message = args
            if callable(message):warn("Passing a callable message to register_action is deprecated; create a LogFormatter subclass instead.",category=RemovedInWagtail217Warning)
            formatter_cls = type('_LogFormatter', (LogFormatter, ), {'label': label, 'message': message})
            register_formatter_class(formatter_cls)
        else:
            # register_action has been invoked as a @register_action(action) decorator; return the function that will register the class
    ...
    def get_action_label(self, action): return self.formatters[action].label
    def get_log_model_for_model(self, model):
        self.scan_for_actions()
        for cls in model.__mro__:
            log_entry_model = self.log_entry_models_by_type.get(cls)
            if log_entry_model: return log_entry_model
    ...
    def log(self, instance, action, user=None, uuid=None, **kwargs):
        ...
        if log_entry_model is None: return
...
def log(instance, action, **kwargs): return registry.log(instance, action, **kwargs)
