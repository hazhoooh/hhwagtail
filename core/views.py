...
def serve(request, path):
    ...
    page, args, kwargs = site.root_page.specific.route(request, path_parts)
...
def authenticate_with_password(request, page_view_restriction_id, page_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not url_has_allowed_host_and_scheme(return_url, request.get_host(), request.is_secure()): return_url = settings.LOGIN_REDIRECT_URL
            restriction.mark_as_passed(request)
            return redirect(return_url)
    else: form = PasswordViewRestrictionForm(instance=restriction)
