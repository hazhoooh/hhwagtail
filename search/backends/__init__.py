...
def get_search_backend_config():
    search_backends = getattr(settings, 'CONSOLE_SEARCH_BACKENDS', {})
...
def get_search_backend(backend='default', **kwargs):
    ...
    try:
        # Try to get the CONSOLE_SEARCH_BACKENDS entry for the given backend name first
...
def _backend_requires_auto_update(backend_name, params):
    ...
    # _CONSOLE_SEARCH_FORCE_AUTO_UPDATE is only used by Wagtail tests. It allows
    # us to test AUTO_UPDATE behaviour against Elasticsearch without having to
    # have AUTO_UPDATE enabed for every test.
    force_auto_update = getattr(settings, '_CONSOLE_SEARCH_FORCE_AUTO_UPDATE', [])
