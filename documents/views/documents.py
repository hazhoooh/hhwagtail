...
class BaseListingView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'q' in self.request.GET:
            self.form = SearchForm(self.request.GET, placeholder="Search documents")
            if self.form.is_valid():
                query_string = self.form.cleaned_data['q']
                documents = documents.search(query_string)
        else:
            self.form = SearchForm(placeholder="Search documents")

        # Pagination
        paginator = Paginator(documents, per_page=100)
...
def add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Document '{0}' added.".format(doc.title), buttons=[
                messages.button(reverse('wagtaildocs:edit', args=(doc.id,)), "Edit")
            ])
...
def edit(request, document_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Document '{0}' updated".format(doc.title), buttons=[
                messages.button(reverse('wagtaildocs:edit', args=(doc.id,)), "Edit")
            ])
            return redirect('wagtaildocs:index')
        else:
    ...
    if local_path:
        ...
        if not os.path.isfile(local_path):
            messages.error(
                ...
                buttons=[messages.button(reverse('wagtaildocs:delete', args=(doc.id,)), "Delete")]
            )
    ...
    return TemplateResponse(request, "wagtaildocs/documents/edit.html", {
        ...
        'form': form,
        'user_can_delete': permission_policy.user_has_permission_for_instance(request.user, 'delete', doc),
    })
...
def delete(request, document_id):
    Document = get_document_model()
    doc = get_object_or_404(Document, id=document_id)
    if not permission_policy.user_has_permission_for_instance(request.user, 'delete', doc): raise PermissionDenied
    if request.method == 'POST':
        doc.delete()
        messages.success(request, "Document '{0}' deleted.".format(doc.title))
        return redirect('wagtaildocs:index')

    return TemplateResponse(request, "wagtaildocs/documents/confirm_delete.html", {'document': doc,})

def usage(request, document_id):
    Document = get_document_model()
    doc = get_object_or_404(Document, id=document_id)
    paginator = Paginator(doc.get_usage(), per_page=100)
    used_by = paginator.get_page(request.GET.get('p'))
    return TemplateResponse(request, "wagtaildocs/documents/usage.html", {'document': doc,'used_by': used_by})
