...
def get_document_model_string():
    ...
    """
    return getattr(settings, 'DOCS_DOCUMENT_MODEL', 'wagtaildocs.Document')


def get_document_model():
    """
    Get the document model from the ``DOCS_DOCUMENT_MODEL`` setting.
    ...
    try:
        ...
        raise ImproperlyConfigured("DOCS_DOCUMENT_MODEL must be of the form 'app_label.model_name'")
    except LookupError:
        raise ImproperlyConfigured(
            "DOCS_DOCUMENT_MODEL refers to model '%s' that has not been installed" % model_string
        )
