...
class AbstractDocument(CollectionMember, index.Indexed, models.Model):
    ...
    def clean(self):
        """
        Checks for DOCS_EXTENSIONS and validates the uploaded file
        ...
        """
        allowed_extensions = getattr(settings, "DOCS_EXTENSIONS", None)
    ...
    @property
    def url(self):
        if getattr(settings, 'RVE_METHOD', None) == 'direct':
            try:
    ...
    def content_type(self):
        content_types_lookup = getattr(settings, 'DOCS_CONTENT_TYPES', {})
        return (
    ...
    def content_disposition(self):
        inline_content_types = getattr(
            settings, 'DOCS_INLINE_CONTENT_TYPES', ['application/pdf']
        )
